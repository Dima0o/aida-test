...
function ProdDell(){
    ...
    $prods = R::find('shop',
        ' token = :token and prod :prod',
        array(
            ':token' => $_COOKIE['PHPSESSID'] ,
            ':prod' => $_POST['item']
        )
    );
    //$prods = R::find('shop', "`token` =? and  prod=?", array("token" => $_SESSION['token'], "prod" => $_POST['item']));
    foreach ($prods as $prod) {
        $cat = R::load('category', $prod->id);
        R::trash($cat);
...
function CardBild(){
   // $pages = R::getAll('shop', 'token = :token , status :status',array(':token' =>  $_SESSION['PHPSESSID'],':status' => 0));
    ...
    $pages = R::find('shop',' token = ? and status=0', array( $_COOKIE['PHPSESSID'] ));

   // $pages = R::find($sql);
    //$rows = R::getAll('shop', 'token = :token , status :status',array(':token' =>  $_SESSION['PHPSESSID'],':status' => 0));
    //var_dump($pages);
    $summ=0;
    if(count($pages)>0){
        ...
        $data=json_encode((super_unique($data,'prod')));
    }else{
        $data='[]';
       // $data='1221';
    }
    return '[{"item":'.$data.',"sum":'.$summ.',"token":"'.$_COOKIE["PHPSESSID"].'"}]';
...
function Start(){
    ...
    if($_SESSION['token']==null or $_SESSION['token']='undefined' or isset($_SESSION['token'])==false){
        TokenOn();
    }

    //работа с остатками
    if($_POST['item']==null or $_SESSION['token']='undefined' or isset($_SESSION['token'])){
        if($_POST['status']==0){
            ProdAdd();
