...
function name($id){
    $qr_result = mysql_query("SELECT *  FROM `prod` WHERE  id=".$id);
    if (mysql_num_rows($qr_result) != 0) {
        while($sql = mysql_fetch_array($qr_result)){
            return  $sql['name'];
        };
...
function price($id)
{
    $qr_result = mysql_query("SELECT `price`.`price`  FROM `prod`,`price` WHERE prod.uid=price.uid AND prod.id='".$id."' ORDER BY `price`.`data` DESC LIMIT 0, 1");
    if (mysql_num_rows($qr_result) != 0) {
        while($sql = mysql_fetch_array($qr_result)){
            return  $sql['price']/10;
        };
